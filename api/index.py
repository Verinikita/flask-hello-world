...
app.config['JSON_FOLDER'] = './archivos_json'
app.config['PDB_FOLDER'] = './archivos_pdb'
...
def AlphaFoldXploR_read(zfile):
    with ZipFile(zfile, 'r') as fz:
        for zip_info in fz.infolist():
            ...
            if tab.endswith(".json"):
                ...
                lista1 = fz.extract(zip_info, app.config['JSON_FOLDER'])
    ...
    with ZipFile(zfile, 'r') as fz:
        for zip_info in fz.infolist():
            ...
            if tab.endswith(".pdb"):
                zip_info.filename = os.path.basename(zip_info.filename)
                lista2 = fz.extract(zip_info, app.config['PDB_FOLDER'])
...
def download_file(filename):
    return send_from_directory(app.config['PDB_FOLDER'], filename, as_attachment=True)
